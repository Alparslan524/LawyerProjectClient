...
export class SignalRService {
  ...
  start(hubUrl: string) {
    if (!this.connection || this._connection?.state == HubConnectionState.Disconnected) {
      ...
      hubConnection.start().then(() => console.log("Connected")).catch(error => setTimeout(() => this.start(hubUrl), 2000));
      //Eğer connection işlemi başarılı değilse her 2 saniyede bir başarılı olasıya kadar kontrol et.
